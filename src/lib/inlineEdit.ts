 ...
 *
 * A document is addressed by id — `projects.3.description` — because a
 * collection has many and the page is showing particular ones.
 *
 * `caseStudies` rather than `case-studies`: the prefix is a path segment, and
 * keeping it one plain word keeps the key readable. Media and Users are
 * deliberately absent — an upload store and an account table hold no prose.
 */
